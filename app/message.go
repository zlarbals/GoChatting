...
func backUpMessages(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	//몽고DB 세션 생성
	session := mongoSession.Copy()
	//몽고DB 세션을 닫는 코드를 defer로 등록
	defer session.Close()
	...
	var messages []Message
	//생성된 날짜 순으로 정렬하여 모든 message 조희
	err := session.DB("test").C("messages").
		Find(bson.M{"room_id": bson.ObjectIdHex(ps.ByName("id"))}).Sort("created_at").All(&messages)
	...
	fileLocation := "requestfile/backupMessages.txt"
	backUpFile, _ := os.Create(fileLocation)

	//defer backUpFile.Close()
	//backup file 삭제
	//defer os.Remove("backupMessages.txt")

	for _, value := range messages {
		fmt.Fprintf(backUpFile, "[%s-%s]\n %s\n", value.User.Name, value.User.Email, value.Content)
	}

	receiverEmail:=ps.ByName("email")
	err = sendEmail(fileLocation,receiverEmail)
	...
	renderer.JSON(w, http.StatusOK, "success")
}
